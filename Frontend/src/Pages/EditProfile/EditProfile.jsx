...
const EditProfile = () => {
  ...
  const handleRemoveEducation = (e, tid) => {
    const updatedEducation = form.education.filter((item, i) => {
          ...
          >
            ...
            <Tab eventKey="education" title="Education">
              {form?.education?.map((edu, index) => (
                <div className="border border-dark rounded-1 p-3 m-1" key={edu?._id || edu?.id || index}>
                  {index !== 0 && (
            ...
            <Tab eventKey="longer-tab" title="Additional">
              ...
              <div className="">
                ...
                {form?.projects?.map((project, index) => (
                  <div className="border border-dark rounded-1 p-3 m-1" key={project?._id || project?.id || index}>
                    <span className="w-100 d-flex justify-content-end">
                      <button
                        className="w-25"
                        onClick={() => {
                          setForm((prevState) => ({
                            ...prevState,
                            projects: prevState.projects.filter((item) => {
                              const currentId = item._id || item.id;
